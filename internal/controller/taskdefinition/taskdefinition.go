...
const (
	...
	errDescribe  = "failed to describe TaskDefinition resource"
	errCreate    = "failed to create TaskDefinition resource"
	errUpdate    = "failed to update TaskDefinition resource"
	errDelete    = "failed to delete TaskDefinition resource"
	errMapping   = "failed to map TaskDefinition resource"
)

// Setup adds a controller that reconciles TaskDefinition managed resources.
func Setup(mgr ctrl.Manager, o controller.Options) error {
	o.Logger.Info("Setup Controller: TaskDefinition")
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(v1alpha1.TaskDefinitionGroupVersionKind),
		managed.WithExternalConnecter(&connector{
			kube:         mgr.GetClient(),
			usage:        resource.NewProviderConfigUsageTracker(mgr.GetClient(), &apisv1alpha1.ProviderConfigUsage{}),
			newServiceFn: clients.NewTaskDefinitionService,
...
type connector struct {
	...
	newServiceFn func(creds []byte) (clients.TaskDefinitionService, error)
}
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	...
	cr, ok := mg.(*v1alpha1.TaskDefinition)
	if !ok {
		return nil, errors.New(errNotTaskDefinition)
	}
...
type external struct {
	...
	service clients.TaskDefinitionService
	logger  logging.Logger
}
...
func (c *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	cr, ok := mg.(*v1alpha1.TaskDefinition)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errNotTaskDefinition)
	}
	...
	cr.SetConditions(xpv1.Available().WithMessage("TaskDefinition exists"))

	observedCompareable, err := c.service.MapToTaskDefinitionCompare(observed)
	if err != nil {
		return managed.ExternalObservation{}, errors.Wrap(err, errMapping)
	}

	specCompareable, err := c.service.MapToTaskDefinitionCompare(&cr.Spec.ForProvider)
	if err != nil {
		return managed.ExternalObservation{}, errors.Wrap(err, errMapping)
	}
...
func createUniqueIdentifier(app *v1alpha1.TaskDefinitionParameters) string {
	return app.Name
}
...
func (c *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	cr, ok := mg.(*v1alpha1.TaskDefinition)
	if !ok {
		return managed.ExternalCreation{}, errors.New(errNotTaskDefinition)
	}

	err := c.service.CreateTaskDefinition(ctx, &cr.Spec.ForProvider)
...
func (c *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	...
	cr, ok := mg.(*v1alpha1.TaskDefinition)
	if !ok {
		return managed.ExternalUpdate{}, errors.New(errNotTaskDefinition)
	}

	err := c.service.UpdateTaskDefinition(ctx, &cr.Spec.ForProvider)
...
func (c *external) Delete(ctx context.Context, mg resource.Managed) error {
	...
	cr, ok := mg.(*v1alpha1.TaskDefinition)
	if !ok {
		return errors.New(errNotTaskDefinition)
	}

	err := c.service.DeleteTaskDefinition(ctx, &cr.Spec.ForProvider)
